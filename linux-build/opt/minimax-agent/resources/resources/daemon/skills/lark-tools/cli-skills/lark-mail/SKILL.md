...
- **`--as user`（推荐）**：以当前登录用户的身份访问其邮箱。在 mavis 中，daemon onboard 已经自动跑过 `lark-cli auth login --recommend` 拿到推荐 scope 全集（含 mail），通常无需再手动登录；若仍报 `LARK_USER_AUTH_REQUIRED` 或缺特定 mail scope，按 `cli-skills/lark-shared/SKILL.md` §Agent 代理发起认证 的 canonical 模板执行增量授权。
