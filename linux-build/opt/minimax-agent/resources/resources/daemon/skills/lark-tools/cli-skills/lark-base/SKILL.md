...
- **`--as user`（推荐）**：以当前登录用户身份操作其有权访问的 Base。在 mavis 中，daemon onboard 已自动跑过 `lark-cli auth login --recommend` 拿到推荐 scope 全集（含 base 常用读写 scope），通常无需再手动登录；若仍提示缺特定 base scope（如 `base:record:update`），按 `cli-skills/lark-shared/SKILL.md` §Agent 代理发起认证 的 canonical 模板执行增量授权。
