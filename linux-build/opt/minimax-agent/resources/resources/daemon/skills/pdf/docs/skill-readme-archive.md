...
`scripts/read_pdf_vision.py` additionally requires the local mavis daemon
to be running with the matrix MCP authenticated (set `MAVIS_ACCESS_TOKEN`
and `MATRIX_BASE_URL` in `~/.zshrc`, then restart the daemon). See
